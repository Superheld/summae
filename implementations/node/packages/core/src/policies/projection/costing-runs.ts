 ...
 */
export class CostingRunsProjection {
  constructor(private readonly runs: CostingRunRepository) {}

  compute(params: Record<string, unknown>): { runs: Array<Record<string, unknown>> } {
    ...
    const rows = this.runs
      .all()
      .filter((run) => fiscalYear === null || run.period.fiscalYear === fiscalYear)
      .filter((run) => period === null || run.period.period === period)
      .map((run) => ({
        runId: run.id.value,
