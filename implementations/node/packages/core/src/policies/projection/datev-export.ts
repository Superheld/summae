...
export class DatevExportProjection {
  ...
  private splitEntry(entry: JournalEntry): Array<Record<string, unknown>> {
    ...
    for (const contra of contraLines) {
      ...
      rows.push({
        // The batch carries the amount without a sign ("Umsatz ohne Soll/Haben-Kz"): direction comes
        // from the indicator, and a cancellation is marked as Generalumkehr rather than by flipping the
        // side or sending a negative number. Since this core reverses by general reversal (negated
        // amount, unchanged side), abs() alone made the reversal byte-identical to the posting it
        // cancels — an import doubled the turnover instead of clearing it. The marker tells them apart.
