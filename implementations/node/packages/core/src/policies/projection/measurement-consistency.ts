import type { CostingRunRepository } from '../../port.js';
...
export class MeasurementConsistencyProjection {
  constructor(private readonly runs: CostingRunRepository) {}

  compute(_params: Record<string, unknown>): Record<string, unknown> {
    const rows: BasisRow[] = [];
    const withoutBasis: Array<Record<string, unknown>> = [];
    ...
    for (const run of this.runs.all()) {
      ...
      if (run.productionCost === null) {
        withoutBasis.push(where);
        continue;
      }

      const included: string[] = [];
      const elected: string[] = [];
      for (const component of run.productionCost.components) {
        if (!component.included) continue;
        included.push(component.id);
        // The election is the optional half. A mandatory component is in the basis because the pack
        // says so, and reporting it as a choice would blame the preparer for the jurisdiction — the
        // two must stay distinguishable, or a pack that promotes a component from optional to
        // mandatory reads as the tenant having changed its mind.
        if (component.treatment === 'optional') elected.push(component.id);
      }

      // Sorted by code point, like every other list this library emits: the basis is a SET, and
      // comparing two of them must not depend on the order the components were configured in. Two
      // runs with the same components in a different order are the same measurement.
      included.sort();
      elected.sort();

      rows.push({ ...where, included, elected });
    }

    const changes: Array<Record<string, unknown>> = [];
    for (let i = 1; i < rows.length; i += 1) {
      const previous = rows[i - 1] as BasisRow;
      const current = rows[i] as BasisRow;

      const added = current.included.filter((id) => !previous.included.includes(id));
      const removed = previous.included.filter((id) => !current.included.includes(id));
      ...
      changes.push({
        ...
        added,
        removed,
        // Reported, never judged. A change inside one fiscal year is still absorbed by that year's
        // own result; a change that spans a year boundary is the one a reader has to account for,
        // because the two years are then not comparable without an explanation. Which of the two a
        // framework cares about is the pack's business — the flag exists so a caller does not have
        // to derive it from two period numbers.
        acrossFiscalYears: previous.fiscalYear !== current.fiscalYear,
      });
    }

    return { runs: rows, withoutBasis, changes, consistent: changes.length === 0 };
  }
}
