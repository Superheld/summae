...
| `appropriateResult` | Geschäftsjahr + Beschlussdatum + Verwendungen je Ziel → Buchung (Ergebnisverwendung, F-CORE-024/SF-25) | `E_APPROPRIATION_UNSUPPORTED`, `E_APPROPRIATION_EXCEEDS_RESULT` |
...
- **`appropriateResult` (2026-08-26):** Die Ergebnisverwendung ist ein **Beschluss**, keine Rechnung — wer entscheidet und ob ausgeschüttet, in Rücklagen eingestellt oder vorgetragen wird, kann eine Bibliothek den Büchern nicht entnehmen; sie ist außerdem auf den Beschlusstag datiert, der regelmäßig im **Folgejahr** liegt. Deshalb bucht sie kein Jahresabschluss, sondern eine eigene Operation: der Aufrufer nennt die Entscheidung (`appropriations[]` mit `target` + Betrag), das **Pack** liefert die Konten (`resultAppropriation`-Modul: `allocationAccount` + `targets`). Welche Ziele es gibt, ist die Antwort des Packs — `de` bietet `carryForward` und `distribution`, `us` und `default` nur `carryForward`; ein nicht angebotenes Ziel ist `E_APPROPRIATION_UNSUPPORTED` und nennt die angebotenen. Verwendbar ist das **noch nicht verwendete** Ergebnis: kumuliertes Ergebnis aller Jahre ≤ `fiscalYear` minus Saldo der `result_allocation`-Konten über den ganzen Bestand — dieselbe Zahl, die die Bilanzposition mit `includesNetIncome` ausweist. Mehr ist `E_APPROPRIATION_EXCEEDS_RESULT`, weniger ist erlaubt (`remaining` sagt, was bleibt). Die Richtung folgt den Büchern: Gewinn → Verwendungskonto im Soll, Verlust umgekehrt; Beträge sind immer positiv. Die entstehende Buchung ist eine **normale** Buchung (korrigierbar, stornierbar, auditiert) — anders als Maschinenbuchungen wird sie **nicht** sofort festgeschrieben, denn sie ist die Eingabe eines Anwenders.
- **`unappropriatedResult` + `setEntityProfile` (2026-08-27):** Die Zahl, gegen die `appropriateResult` prüft, existierte nur im Fehlerfall — sie verließ die Bibliothek als `available`-Detail einer Absage. Die Projektion `unappropriatedResult` veröffentlicht sie: den Topf als Ganzes (`cumulativeResult`/`appropriated`/`unappropriated`) und je Geschäftsjahr dessen Herkunft samt `available` — **derselben Funktion**, die die Operation prüft, nicht einer zweiten Implementierung davon. `setEntityProfile` setzt dazu Rechtsform und Größenklasse gegen den Pack-Katalog (Modulsorte `legalForms`), und die Projektion meldet daraus `resolutionRequired` und `resolutionDueBy` je Jahr. `false` (diese Form beschließt nichts) und `null` (niemand hat gesagt, was diese Firma ist) sind verschiedene Antworten. Überwacht wird die Frist außerhalb — „die Daten müssen…" ist Paket, „der Nutzer muss bis X…" ist App.
