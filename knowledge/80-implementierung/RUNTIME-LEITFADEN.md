...
Spec-Lücke/Widerspruch beim Bauen → **nicht raten, nicht Fixture biegen.** In der Repo-Wurzel an `FINDINGS-OPEN.md` anhängen — **ein Register für beide Implementierungen** seit 2026-08-25, getrennt nach *Zustand*: `FINDINGS-OPEN.md` = offen (kurz genug, um es ganz zu lesen), `FINDINGS-CLOSED.md` = entschieden, in voller Länge, mit der Status-Tabelle. Schließen heißt: Block hinüber verschieben. Die Dateien unter `implementations/<sprache>/` sind nur noch Zeiger — als `SPEC-nnn` (Widerspruch in Spec/Fixture/Modell) oder `IMPL-nnn` (Implementierungsdefekt), mit Statuszeile in der Tabelle oben (Was / Wo / Gewähltes Verhalten / Vorschlag). Das Präfix `F-0xx` gibt es seit 2026-08-23 nicht mehr; es saß im Namensraum der Anforderungen. `80-implementierung/SPEC-FINDINGS.md` ist das **geschlossene historische** Register der ersten Runde und hat eigene, kollidierende Nummern — dort nichts mehr anhängen und mit dem nächstplausiblen Verhalten weiterbauen. Die Wissensbasis arbeitet Findings als nächste Spec-Version ein und löst eine Retrofit-Welle für **alle** Runtimes aus (so geschehen mit SPEC-001…SPEC-009 aus PHP). **Node-typische Findings sind v. a. bei Rundung/Zahlentypen erwartbar** — genau dort härtest du den Determinismus-Anhang.
