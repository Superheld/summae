...
> **Historischer Meilenstein-Snapshot (Stand 2026-06-08).** Die Zahlen unten (40/43
> Fixtures, „M4 = Laravel + CLI", Node noch in der Zukunft) sind der damalige Stand
> und bewusst nicht nachgezogen. **Aktuell:** 45 Fixtures, Node-Runtime fertig, SF-15
> bidirektional grün — siehe `RUNTIME-LEITFADEN.md` und `testing/testsuite/abdeckung.md`.
> Dauerhaft gültig bleiben hier die **NF-7-Performancezahlen** und die **7 Adapter-
> Annahmen** (auf die der Leitfaden verweist).
...
Die Findings SPEC-001…SPEC-007 wurden von der Wissensbasis aufgegriffen (v0.5,
...
|---|---|---|
| SPEC-001 | Kein Code für *unbekannte* voucherId | eigener Code `E_VOUCHER_UNKNOWN` (Referenzschritt) |
| SPEC-002 | `E_ENTRY_NOT_FINALIZED` widersprüchlich | gestrichen; `reverse` statusunabhängig; `closePeriod` ohne Festschreibungspflicht |
| SPEC-003 | Kein Code für Jahresabschluss-Guard | eigener Code `E_FISCALYEAR_UNFINALIZED_ENTRIES` |
| SPEC-004 | Asset-Konten unspezifiziert | Regelmodul-Block `assetAccounts` (Heuristik entfernt) |
| SPEC-005 | Manifest-Streams/Version widersprüchlich | `streams`+`hashAlgorithm` im Schema, `auditLog` immer, `formatVersion` aktuell |
| SPEC-006 | `E_COSTING_RUN_UNKNOWN` fehlte | im Katalog (entsprach bereits der Implementierung) |
| SPEC-007 | Bilanz-Seitenzuordnung fragil | `side: assets\|liabilitiesAndEquity` am Wurzelknoten |
