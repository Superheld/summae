...
Documented contradictions between spec / fixture / model, and defects found while building
(root `CLAUDE.md`: "don't guess, don't bend the fixture, but document it and build on with the
next-most-plausible behaviour").

> **One register for both implementations, since 2026-08-25.** It used to be two files —
> `implementations/php/SPEC-FINDINGS.md` and `implementations/node/SPEC-FINDINGS.md` — and the
> language-neutral findings lived in both. Seven `SPEC-` entries were byte-identical copies and
> **SPEC-014 had already drifted**: the PHP copy carried the decision, its reasoning and the
> IMPL-028 note, the Node copy had degenerated into a summary ending in "full write-up on the PHP
> side". That is the failure mode this project refuses everywhere in the code — a copy nothing
> compares drifts — and the register that records such failures should not be an example of one.
>
> The two files are now thin pointers here. Which language a finding was *found* in is a
> sentence in the entry, not a directory: most of them turned out to concern both.
...
Everything below arrived through the second implementation. Most of it concerns **both** —
`IMPL-009`, `IMPL-010`, `IMPL-011` and `IMPL-013` were fixed in PHP as well — which is the
other half of why one register beats two: a finding filed under a language is not a finding
that belongs to it.
