...
| [`open-decisions.md`](open-decisions.md) | ⚠️ **two of four still open — 2026-08-29** | The four questions the HGB work left open where *building* would have been the wrong first move. **Decided:** Fifo/Lifo is not built and the boundary holds (§ 256 permits a simplification, it does not require the books to keep a stock record); the `DocumentStore` port is decided in principle and blocked on three retention questions, with the consequence for the application's A-10 named. **Open, and for a person:** foreign currency (§ 256a) — declare it out of scope or build the only change on the list that starts at the substrate; and single-circle vs. two-circle cost accounting, which hangs on whether the municipal pack is still coming. |
