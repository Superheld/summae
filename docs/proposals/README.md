...
| [`constraint-vocabulary.md`](constraint-vocabulary.md) | ⚠️ **open** | Two decisions, separable. **A:** a predicate keyed on the account's `subtype` instead of its number, so "an exempt sale may not carry output tax" is written once instead of once per pack — recommended, but only after closing the subtype vocabulary, because a prohibition keyed on a free string silently does nothing. **B:** `appliesWhen` on a rule, restricted to `legalForm` and `taxationMethod`; `smallBusiness` and amount conditions are argued **out**, with what would change that. |
