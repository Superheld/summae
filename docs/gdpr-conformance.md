...
| `auditRecord` | `changes` | before/after of the changed fields | mirrors whatever the changed record held, including a partner's name and address |
...
|---|---|---|
| **Art. 15 — access** | ⚠️ **the parts exist, the answer does not** | Everything needed is readable: `partner` for the master data, `openItems`/`accountSheet` for what is owed, `journal` for the postings, `auditLog` for who touched the record (`audit-log-filtered`). What does not exist is *one* operation that assembles them for one data subject. Today the application joins four projections itself, and each one that forgets a source produces an incomplete disclosure. Named in §7. |
...
| **Art. 17 — erasure** | ➖ **and the reason matters** | Art. 17(3)(b) exempts processing required by a legal retention duty, and § 147 AO / § 257 HGB impose one on everything in the books — ten years, six for some classes. So the journal is not erasable, by law, and its append-only design agrees with the law rather than fighting it. **But summae also cannot erase what is *not* covered by that duty** — a partner created by mistake that no entry ever referenced. There is no `deletePartner` at all. That is the honest gap, and it is in §7. |
...
2. **The exemption covers the books, not everything adjacent.** A partner record that never entered
   ...
   undiminished. summae cannot execute it (§7).
...
|---|---|---|---|
| 1 | **No erasure for a partner the books never referenced** | ⚠️ **open, and the clearest gap** | There is no `deletePartner`, deliberately — *"an id that open items point at must not vanish"*, which is right. But the rule is applied to every partner, not only to referenced ones, so a record created by a typo and never posted against is kept forever with no legal basis. The shape of the fix is visible: erasure permitted exactly when no journal entry, open item, voucher or audit record references the id, refused with a named error otherwise. It needs a new operation, a fixture per language and an error code in both catalogues — a job, not an afternoon. |
