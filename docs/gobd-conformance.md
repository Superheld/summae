...
| **Every state-changing operation leaves a trace** | ✅ | `F-CORE-014`/`F-CORE-020`, fixture `core/audit-trail-period-and-config` **plus** the enumerating guard `AuditTrailContractTest` / `audit-trail-contract.test.ts`: every state-changing dispatcher operation is run for real and must produce an audit record. This is stronger than fixture coverage — see §11. |
...
| Obligation | Status | Proof |
|---|---|---|
| Technical system description generated by the package | ⚠️ **open** | `F-IO-007` is a MUST in the requirement list, has no fixture, no standard case, and no implementation. Scope: a projection assembling format version, operations and projections, enforced invariants, error codes, the pack in use and its module versions. |
...
|---|---|---|---|
| 1 | `F-IO-007` technical system description | ⚠️ MUST, unimplemented | Scoped in §11; a projection, no data-format change. The most clearly buildable of the open items. |
...
| 4 | Ten operations write no audit record of their own | ⚠️ known, listed | `settle`, `acquireAsset`, `disposeAsset`, `runDepreciation`, `allocate`, `runCosting`, `releaseCosting`, `postVoucher`, `createVoucher`, `importChartOfAccounts`. All of them post through the ledger, so a `journalEntry` trace exists — but nothing says *an asset was acquired*. Named in `UNCOVERED_KNOWN` in both contract tests, so the list cannot grow silently. |
