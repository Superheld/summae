...
| The shipped `de` pack covers every VAT case a German business meets | ⚠️ **three cases short, and none of them is a GoBD obligation** | **Read the status before the list.** The GoBD asks that what is booked be computed correctly at its reference date (row above, ✅) and that nothing filed be silently wrong (row above, ✅). It does *not* require a pack to carry every code in the UStG — a business with no OSS turnover is not less GoBD-compliant for a pack that cannot express one. What is missing is therefore **product scope**, named here so it is not mistaken for covered, and scoped in [`proposals/de-pack-vat-completeness.md`](proposals/de-pack-vat-completeness.md) (project backlog: P8): **§ 15a** input-tax adjustment (a multi-year correction with its own state — an expansion policy, not a code), **§ 14c** incorrectly shown tax (a tax amount with no base, which the current code shape cannot express: every code computes tax *from* a base), and **OSS** (destination-country rates and a separate return). Each is a feature, not a gap in a data file. **What this row said until 2026-08-28 was stale, and how it got that way is the part worth keeping:** it named intra-community *acquisition* and exempt *export* as missing, and both were built the very same day, hours after the row was written (`IGE19`/`IGE7`/`AUSFUHR`, fixtures `de-ig-erwerb` and `de-ausfuhr`). The census claim was prose and nothing held it against the pack, so it survived five days of green builds describing a product that had moved. Prose in a census is a claim like any other — see §15, whose table the document's own gate now holds against the shipped manifests. |
| The same document entered twice is **detectable** | ✅ **since 2026-08-28**, and detection is all it is | `F-CORE-044`, fixture `core/duplicate-vouchers` — projection `duplicateVouchers` groups vouchers by document identity (the issuer, `partnerId` where there is one and the free-text `issuer` otherwise, plus the number) and reports every group with more than one member. **The defect it answers is invisible everywhere else:** an incoming invoice booked a second time produces a second voucher, a second balanced entry and a second input-tax deduction while every invariant in this document holds — the entries balance, both carry a voucher, both sit in an open period, the trial balance adds up. **Deliberately not a refusal:** `voucherNumber` carries no uniqueness and must not, because two suppliers legitimately send the same invoice number; a hard rule would be wrong rather than merely strict. So this row is ✅ for *detectable* and says nothing about prevention — acting on the list is the application's workflow, the same division `unfinalizedEntries` draws in §5. A duplicate already corrected by reversal stays in the list at `postedTotal: 0.00`, because a list read to decide whether something was corrected has to show that it was. |
...
| **Valuation events are traceable as valuation events** | ✅ **since 2026-08-29** | Same enumerating guard, twelve cases more: `inventoryValuation/valued`, `provision/recognized`\|`used`\|`released`\|`remeasured`, `deferral/recognized`, `deferralRelease/completed`, `inputTaxAdjustment/adjusted`, `asset/writtenDown`\|`writtenUp`\|`usageReported`\|`specialDepreciationBooked`. This row exists because the entries these operations produce are *machine* entries, finalized on creation and therefore never correctable — so the trail is the only place that records **who** decided a provision was owed and on what estimate. The figures are in the books either way; the decision behind them is not. Each aggregate is persisted behind its own repository in both adapters, which is the lesson of the costing-run row below applied before it could bite: an audit record must point at something that survives a restart. |
...
| Obligation | Status | Proof |
|---|---|---|
| Machine-evaluable data set | ✅ | `F-IO-001`, fixture `io/journal-export-z3-current` — `journalExport` produces journal, accounts, vouchers, partners and auditLog with SHA-256 content hashes per stream and a field catalogue (name, type, meaning) that describes **every** field of **exactly** the streams on the carrier. The completeness half is guarded separately by `FieldCatalogCompletenessTest`/`field-catalog-completeness.test.ts`, because the fixture pins `fieldCatalogIncluded` — a boolean — and the catalogue sits outside the content hashes, so it drifted for as long as it existed (IMPL-038). |
...
| 5b | ~~VAT correction on a consideration-reducing settlement~~ | ✅ **closed 2026-08-28**, with the half that stays the app's named | **Was:** neither built nor documented, and the gap between "summae records the allocation" and "the books are right" was invisible. **Now:** the shipped `de` pack declares `accountCombinationRules` (§4, `de-entgeltminderung-erzwungen-current`), so the *posting* that reduces the consideration is refused unless it carries its VAT correction — both directions, output and input. `settle` itself remains unconstrained and will: it records an allocation and posts nothing, so there is no entry for a predicate to see. That residue stays **A-13** on the app's list, and this row is ✅ for the half a library can hold. |
