...
| Costing runs are traceable | ⚠️ **partly — corrected 2026-08-23** | The audit records exist (`costingRun/created`, `costingRun/released`, same guard) and they are persisted. **The runs themselves are not.** `CostingService` keeps them in memory — there is no `CostingRepository` among the ports — so after a restart a durable audit record points at an object that no longer exists. Costing has no journal effect, so no books are wrong and this is not a GoBD matter; but the row said more than it could keep, and a ✅ that overstates is worse than an honest ⚠️. Scoped in the project backlog: it needs a port, an in-memory and two database adapters, and a table — hence a package of its own rather than a fix. |
