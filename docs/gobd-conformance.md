...
| Costing runs are traceable | ⚠️ **partly — corrected 2026-08-23** | The audit records exist (`costingRun/created`, `costingRun/released`, same guard) and they are persisted. **The runs themselves are not.** `CostingService` keeps them in memory — there is no `CostingRepository` among the ports — so after a restart a durable audit record points at an object that no longer exists. Costing has no journal effect, so no books are wrong and this is not a GoBD matter; but the row said more than it could keep, and a ✅ that overstates is worse than an honest ⚠️. Scoped in the project backlog, and the scoping turned out to understate it. The port, the adapters and the table are mechanical; the blocker is that **summae has no way to evolve a shipped schema at all.** Both languages create their tables once, at workspace initialisation (`SchemaInstaller::create`, `installSchema`), and nothing upgrades an existing one — PHP could add a second Laravel migration, Node has no migration concept whatsoever. Costing is simply the first thing that needs one. Deciding between versioned migrations in both languages, an idempotent "ensure schema" on startup, or a frozen 0.x schema is an architecture decision with long consequences, so it is named rather than guessed. |
...
| Obligation | Status | Proof |
|---|---|---|
| Technical system description generated by the package | ✅ | `F-IO-007`, fixture `io/system-description-invariants` — the `systemDescription` projection reports format version, the tenant's tax profile, the eleven enforced invariants *with the mechanism that enforces each*, every audit event kind, and an explicit `notProvided` list. Byte-identical in both languages. That it also reports the **full** API surface is proven where it can be proven: `tenant-operations-contract.test.ts` / `TenantOperationsContractTest` hold the published list against the dispatcher's own routing table in both directions, so a capability that exists and is not described fails the build. A fixture could only hold a hand-copied list, and did until 2026-08-24 — at the price of going red whenever summae gained a capability. |
...
| 6 | The constraint policy kind has a socket, with one predicate | ⚠️ partly | **Was:** no module kind carried a constraint at all, so a jurisdiction could contribute rules and views but never a prohibition, and any GoBD rule that *is* a constraint would have had to go into the core. **Now:** `constraint` is a module kind and a pack can declare which accounts may not be posted without which dimension (`dimensionRules`, proven by `xx-5-constraint-dimension-rule`). That is one predicate, not a general one: a pack still cannot express "this settlement must be accompanied by a tax correction" (§4, A-13) or a deadline. The row stays ⚠️ until a constraint can be written about something other than dimensions — the shape of the socket is settled, its vocabulary is not. |
