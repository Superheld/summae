...
| The description names the pack the tenant runs on | ⚠️ **open** | A tenant does not retain its manifest — the factory resolves a pack into a rule bundle and keeps the bundle, not the identity. State pack and version by hand until that plumbing exists in both languages. |
