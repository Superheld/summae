...
| § 252 Abs. 1 Nr. 3 — individual measurement at the reporting date | 🟡 | Assets are measured individually (`assets/*`), and each one carries its own plan, write-down and write-up. **Stock is not:** `valuateInventory` values a *category* at one unit value, which is § 240 Abs. 4 group measurement rather than individual measurement — the sentence that stood here until 2026-08-29 (*"untestable for stock while stock does not exist"*) went stale the day stock was built. Whether the unit value came from an individual measurement is decided where the goods movements are, and summae records which basis was used rather than reproducing the count. |
...
| § 275 Abs. 2 — **income statement, Gesamtkostenverfahren** | ✅ for a business without financial assets | `pack/de-pack/de-guv-vollstaendig`. Twelve positions now: changes in inventory (Nr. 2), own work capitalised (Nr. 3), interest received and paid, income taxes and other taxes. What is still absent is the block around financial assets — income from participations and securities, write-downs on financial assets (Nr. 9–11, 13) — and that is not a mapping gap: the shipped `de` chart has no financial-asset accounts for them to point at. A pack that adds those accounts adds those positions with them. |
...
| **6** | **Consumption sequence (§ 256, § 6 Abs. 1 Nr. 2a EStG)** | ~~open question~~ **decided 2026-08-29: not built** | Closed by deciding rather than by building, which is what the row asked for. The criterion settles it: § 256 **permits** a consumption sequence as a simplification of individual measurement, it does not require the books to keep a stock record — so summae does not hold one. The unit value stays an input, and `measurementConsistency` catches the year somebody switches basis without saying so. The cost is named rather than discovered: a business whose only stock system *is* its accounting system cannot run Fifo inside summae. Full argument, and what would reopen it, in [`proposals/open-decisions.md`](proposals/open-decisions.md) §1. |
...
| **11** | **Foreign currency (§ 256a)** | decision first — **and the question is now written down** | Both options are costed in [`proposals/open-decisions.md`](proposals/open-decisions.md) §3, with a recommendation (declare it out of scope for now, build it when a real embedder needs it) and the reason that is not about cost: building it without a concrete case would fix the *design* of multi-currency around a guess, and the seam it has to cut — which rate, whose date, per line or per entry — is exactly what a real case decides. **This one waits on a person.** |
...
It is **not a claim that summae is unsuitable for bookkeeping** — and on the day it was written that
sentence had to do more work than it does now. It then read: every ⚠️ concerns a business that holds
stock, forms provisions, invoices in foreign currency, or files under § 266. Three of those four are
✅ as of the same day. What is left is the one that starts at the substrate.

**What the document looks like after 2026-08-29, stated once so the rows above do not have to be
counted.** Ten of the twelve open rows were built; two were decided rather than deferred (Fifo/Lifo
not built, with the criterion applied; the offsetting prohibition half built, with the other half
shown to have no surface). What genuinely remains is **foreign currency** (§ 256a), a decision before
it is a task, and a short list of 🟡 rows where the shape is complete and one part is not: no
reclassification between asset positions, no allowance on receivables, equity as one position rather
than five, no Umsatzkostenverfahren, no hedging units, no contingent liabilities below the line.
Each is named above and none of them is a class of thing the balance sheet cannot hold.
